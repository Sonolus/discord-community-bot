...
export const articleCommands = [...contents.entries()].map(([locale, { articles }]) => {
    const data = new SlashCommandBuilder().setName(locale).setDescription(`Articles for ${locale}`)
        ...
        data.addSubcommand((builder) => {
            ...
            ;[...Array(count).keys()].forEach((i) =>
                builder.addUserOption((option) =>
                    option.setName(`user-${i + 1}`).setDescription('User to mention'),
                ),
            )
